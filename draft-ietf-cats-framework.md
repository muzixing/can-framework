...
Computing service architectures have evolved from single service site to multiple, sometimes collaborative, service sites to address various issues, such as long response times, or suboptimal utilization of service and network resources.
...
Computing metrics: 
  : Computing metrics are the metrics come from computing side, includes but not limited to, compute-related metrics, network-related metrics, storage-related metrics, even the energy cost related metrics {{?I-D.ietf-cats-metric-definition}}.
...
~~~
{: #fig-cats-example-overlay title="An Example of CATS Metric Distribution in a Distributed Model"}
